...
{
    [Area("AdminPanel")]
    public class HomeShippingController : Controller
    {
        private readonly AppDbContext _context;
        private readonly IWebHostEnvironment _webHostEnvironment;

        public HomeShippingController(AppDbContext context, IWebHostEnvironment webHostEnvironment)
        {
            _context = context;
        ...
        public async Task<IActionResult> Create(HomeShipping ShippingVM)
        {
            ...
            if (!ShippingVM.Photo.ContentType.Contains("image/"))
            {
                return View();
            }

            if (ShippingVM.Photo.Length / 1024 > 5000)
            {
                return View();
            }
            string filename = Guid.NewGuid().ToString() + "___" + ShippingVM.Photo.FileName;
            string path = Path.Combine(_webHostEnvironment.WebRootPath, "images/dbphoto", filename);

            using FileStream stream = new(path, FileMode.Create);

            await ShippingVM.Photo.CopyToAsync(stream);

            HomeShipping shipping = new()
            {
                Label = ShippingVM.Label,
                Description= ShippingVM.Description,
                Image = filename
            };
